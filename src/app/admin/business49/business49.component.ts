 ...
 */
@Component({
  selector: 'app-business49',
  templateUrl: './business49.component.html',
  styleUrls: ['./business49.component.css'],
})
export class Business49Component implements OnInit {
  adminLnbRef?: AdminLnbComponent;

  form: FormGroup;

  constructor(route: ActivatedRoute, private router: Router, private service: CmmnCodeService) {
    ScUtil.loadStyle('../assets/css/business49.css');
  ...
  onDeleteClick(): void {
    ...
    this.service.deleteById(this.form.controls.cmmnCodeId.value).subscribe(() => {
      this.onListClick();
    });
  }

  onUpdateClick(): void {
    this.router.navigate(['admin/business50', this.form.controls.cmmnCodeId.value]);
  }
}
