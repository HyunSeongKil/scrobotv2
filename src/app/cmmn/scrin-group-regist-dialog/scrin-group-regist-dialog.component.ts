...
export class ScrinGroupRegistDialogComponent implements OnInit {
  ...
  @Output() scrinGroupRegistingEvent = new EventEmitter<any>();
  @Output() scrinGroupRegistedEvent = new EventEmitter<any>();
  ...
  open(prjctId: string, scrinGroups: Scrobot.ScrinGroup[]) {
    ...
    this.modalService.open(this.content, { ariaLabelledBy: 'modal-basic-title' }).result.then(
      (result: any) => {
        ...
        this.scrinGroupRegistingEvent.emit('');

        //
        this.service.regist(this.form.value as Scrobot.ScrinGroup).then((res: any) => {
          this.scrinGroupRegistedEvent.emit(res.data);
        });
      },
      (reason: any) => {
